...
basicConfig(format='[%(asctime)s - %(levelname)s] %(message)s',
            datefmt="%H:%M:%S")
logger = getLogger("DDOS_INFINITY_X")
...
if __name__ == '__main__':
    ...
    with suppress(KeyboardInterrupt):
        with suppress(IndexError):
            ...
            show_attack_banner(method, str(target or url.host), threads, timer)
            logger.info(
                f"{bcolors.WARNING}[%s] Attack Started ->{bcolors.OKBLUE} %s{bcolors.WARNING} | method:{bcolors.OKBLUE} %s{bcolors.WARNING} | {bcolors.OKBLUE}%ds{bcolors.WARNING} | threads:{bcolors.OKBLUE} %d{bcolors.WARNING}{bcolors.RESET}"
                % (__brand__, target or url.host, method, timer, threads))
            event.set()
            ts = time()
            while time() < ts + timer:
                logger.debug(
